...
COMMON_AUDIO_OUTPUT_KEYS = ("audio", "audios", "files", "gifs")
...
def iter_filename_items(value: Any) -> list[dict[str, Any]]:
    """Return dicts with filenames from a ComfyUI output value."""

    if isinstance(value, dict):
        return [value] if "filename" in value else []
    if isinstance(value, list):
        return [item for item in value if isinstance(item, dict) and "filename" in item]
    return []


def iter_audio_output_items(node_output: dict[str, Any]) -> list[dict[str, Any]]:
    """Return common ComfyUI audio/file output items from one history node."""

    audio_items: list[dict[str, Any]] = []
    for key in COMMON_AUDIO_OUTPUT_KEYS:
        audio_items.extend(iter_filename_items(node_output.get(key)))

    for value in node_output.values():
        if isinstance(value, list):
            audio_items.extend(iter_filename_items(value))

    return audio_items


def history_output_keys(history: dict[str, Any], prompt_id: str) -> dict[str, list[str]]:
    """Return output keys present in each ComfyUI history node for debugging."""
...
def extract_audio_output(history: dict[str, Any], prompt_id: str) -> AudioOutput | None:
    ...
    for node_output in outputs.values():
        ...
        for audio_item in iter_audio_output_items(node_output):
...
def wait_for_audio_output(
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            logger.warning(
                "Timed out waiting for ComfyUI audio output for prompt_id %s. "
                "History output keys: %s",
                prompt_id,
                history_output_keys(last_history, prompt_id),
            )
            raise HTTPException(
                status_code=504,
                detail=(
