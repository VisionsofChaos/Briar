...
def test_home_serves_chat_frontend() -> None:
    ...
    assert "url('/static/backgrounds/briar_room.png')" in html
    assert 'id="chat-log"' in html
    assert 'id="message-input"' in html
    assert 'id="briar-canvas"' in html
    assert 'id="briar-portrait"' in html
    assert '/static/visemes/briar_idle.png' in html
    assert "canvas.getContext('2d', { willReadFrequently: true })" in html
    assert 'function renderBriarSprite' in html
    assert 'function chromakeyImageData' in html
    assert 'g > 120 && g > r * 1.25 && g > b * 1.25' in html
    assert 'g > 105 && g > r * 1.12 && g > b * 1.12' in html
    assert "portraitFrame.classList.add('fallback-image')" in html
    assert "let currentMouth = 'idle'" in html
    assert "let isBlinking = false" in html
    assert 'function updateBriarSprite()' in html
    assert 'renderBriarSprite(filename)' in html
    assert "briar_idle_blink.png" in html
    assert "briar_a_blink.png" in html
    assert "briar_e_blink.png" in html
    assert "briar_open_blink.png" in html
    assert "briar_o_blink.png" in html
    assert "const weightedMouths = ['a', 'a', 'a', 'e', 'e', 'e', 'open', 'o']" in html
    assert '90 + Math.floor(Math.random() * 41)' in html
    assert '3000 + Math.floor(Math.random() * 4001)' in html
    assert "fetch('/chat'" in html
    assert "player.play()" in html
    assert "player.addEventListener('play', startLipSync)" in html
    assert "player.addEventListener('pause', stopLipSync)" in html
    assert "player.addEventListener('ended', stopLipSync)" in html
    assert "player.addEventListener('error', stopLipSync)" in html


def test_static_viseme_directory_is_mounted() -> None:
    routes = {getattr(route, "path", None): route for route in main.app.routes}

    assert "/static" in routes
    assert main.STATIC_DIR.name == "static"
    assert (main.STATIC_DIR / "visemes").is_dir()
