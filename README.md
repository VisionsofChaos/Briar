...
The workflow must contain a Qwen3-TTS VoiceClone node with a target text input such as `target_text`. When `POST /tts` is called, Briar loads that workflow, replaces the VoiceClone target text with the incoming request text, and submits the workflow to ComfyUI at `http://127.0.0.1:8188/prompt`.
